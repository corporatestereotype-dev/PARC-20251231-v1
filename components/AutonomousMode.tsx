...
const AutonomousMode: React.FC<AutonomousModeProps> = ({ settings, simulationResult, onUpdate, onReset }) => {
    ...
    // FIX: The selection refs should hold selections of container 'g' elements,
    // which do not have specific data bound to them directly. The data is bound to their children.
    ...
    const runAndMergeSimulation = useCallback(async (previousResult: AutonomousSimulationResult | null) => {
        ...
        try {
            ...
            setStatus('paused');
            // Set playback to the start of the new chunk
    ...
    useEffect(() => {
        ...
        if (simulationResult) {
            ...
            if (status === 'idle') { // Only set to paused if it was idle, don't interrupt running state
                 setStatus('paused');
            }
        } else {
             // If there's no data, start a new simulation automatically
            if(status === 'idle') {
                runAndMergeSimulation(null);
            }
        }
    }, [simulationResult, runAndMergeSimulation, status]);


    // Effect for D3 setup (runs once per view change)
    useEffect(() => {
    ...
    const handleExportAndReset = () => {
        ...
        if (!window.confirm('This will export the current simulation data and then reset the simulation, deleting all progress. Are you sure you want to continue?')) {
            return;
        }
            ...
            try {
                ...
                if (typeof result === 'string') {
                    ...
                    if (parsedData.simulationTitle && parsedData.simulationTimeline && parsedData.generatedUsers) {
                        onUpdate(parsedData);
                        setStatus('paused');
                        setCurrentEventIndex(-1); // Reset playback
                    } else {
            ...
            <>
                <main className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
                    ...
                    <aside className="col-span-3 flex flex-col space-y-4 overflow-y-auto">
                        <h3 className="text-lg font-bold">Simulation Timeline ({simulationData?.simulationTimeline.length} Events)</h3>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex gap-2">
                            <button onClick={() => runAndMergeSimulation(simulationData)} disabled={status === 'loading'} className="bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-2 px-4 rounded-md transition-colors disabled:bg-slate-600 disabled:cursor-not-allowed">
                             ...
                             <button onClick={() => { 
                                 if (window.confirm('Are you sure you want to reset the simulation? All progress will be lost and cannot be recovered.')) {
                                    onReset(); 
                                    setStatus('idle');
                                 }
                             }} disabled={status === 'loading'} className="bg-red-800 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-md transition-colors disabled:bg-slate-600">
                                Reset
                            </button>
                        </div>
                     ...
                     <div className="flex items-center gap-4 pt-3 mt-3 border-t border-slate-700">
                        ...
                        <input
                            ...
                            className="hidden"
                        />
                        <button onClick={() => fileInputRef.current?.click()} className="bg-slate-600 hover:bg-slate-500 text-white font-semibold py-2 px-4 text-sm rounded-md transition-colors">
                            Import Data
                        </button>
                        <button onClick={handleExportAndReset} disabled={!simulationData} className="bg-slate-600 hover:bg-slate-500 text-white font-semibold py-2 px-4 text-sm rounded-md transition-colors disabled:bg-slate-700 disabled:cursor-not-allowed">
                            Export &amp; Reset
                        </button>
