...
const callOllama = async (prompt: string, model: string, systemInstruction?: string): Promise<string> => {
  try {
    ...
    throw new Error("Failed to connect to Ollama. Make sure it's running locally on port 11434.");
  }
};
...
export const getAIResponse = async (prompt: string, settings: Settings, projectContext?: ProjectContext): Promise<string> => {
  ...
  if (settings.aiProvider === 'gemini') {
    ...
    return callOllama(prompt, settings.ollamaModel, systemInstruction);
  }
};

export const getSyntheticUserResponse = async (
    userMessage: string,
    agent: SyntheticUser,
    community: Community,
    ...
    const systemInstruction = `You are playing the role of ${agent.name} in a collaborative online research community.
    
    **Your Persona:** ${agent.personaSummary}
    
    **Community Context:**
    - Name: ${community.name}
    - Theme: ${community.themeDescription}
    
    **Instructions:**
    - Respond to the user's message in character.
    - Keep your response relatively concise (chat message style).
    - Draw upon your specific expertise defined in your persona.
    - Be helpful, collaborative, but maintain your unique voice.`;

    // Construct a simple history string
    const recentHistory = messageHistory.slice(-5).map(m => `${m.user.name}: ${m.text}`).join('\n');
    const prompt = `Recent Chat History:\n${recentHistory}\n\nUser: ${userMessage}\n\nRespond as ${agent.name}:`;

    if (settings.aiProvider === 'gemini') {
        return callGemini(prompt, systemInstruction);
    } else {
         if (!settings.ollamaModel) throw new Error("Ollama model not configured");
        return callOllama(prompt, settings.ollamaModel, systemInstruction);
    }
};
...
export const generateCommunityProfile = async (
    ...
    if (settings.aiProvider !== 'gemini') {
        throw new Error("Community generation currently requires Gemini for its JSON schema capabilities.");
    }
...
- The color palette must be a set of 9 valid CSS hex color codes that match the requested vibe. Crucially, ensure there is high contrast between text colors (--text-primary, --text-secondary, --text-accent) and background colors (--bg-primary, --bg-secondary, --bg-tertiary) for readability. The color names must be exactly as specified in the schema.`;
    ...
    return await callGeminiWithSchema(prompt, systemInstruction, schema);
};
...
export const runSyntheticUserWorkflow = async (
    ...
    if (settings.aiProvider === 'gemini') {
        ...
        users = await callGeminiWithSchema(userGenPrompt, userGenSystemInstruction, schema);

    } else {
        const fullPrompt = `${userGenSystemInstruction}\n\nPROMPT: ${userGenPrompt}`;
        const ollamaResponse = await callOllama(fullPrompt, settings.ollamaModel);
        try {
            const cleanedResponse = ollamaResponse.replace(/```json/g, '').replace(/```/g, '').trim();
            users = JSON.parse(cleanedResponse);
        } catch {
             throw new Error("Ollama did not return valid JSON for synthetic users. Try using a different model.");
        }
    }
    ...
    **Your Task:**
    ...
    let report: string;
     if (settings.aiProvider === 'gemini') {
        report = await callGemini(reportGenPrompt, reportGenSystemInstruction);
    } else {
        report = await callOllama(reportGenPrompt, settings.ollamaModel, reportGenSystemInstruction);
    }
    ...
    if (settings.aiProvider === 'gemini') {
        ...
        try {
            ...
            return JSON.parse(cleanedResponse);
             ...
             throw new Error("Ollama did not return valid JSON for the project team. Try using a different model.");
        }
    }
};
...
export const runAutonomousSimulation = async (settings: Settings, previousResult: AutonomousSimulationResult | null): Promise<AutonomousSimulationResult> => {
    if (settings.aiProvider !== 'gemini') {
        throw new Error("Autonomous mode currently requires the Gemini API for its advanced JSON mode capabilities.");
    }
