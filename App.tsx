...
import { generateProjectTeam, getSyntheticUserResponse } from './lib/ai';
import { applyTheme, DEFAULT_STYLE } from './lib/theme';
import type { User, Project, Settings, View, FeedItem, ChatMessage, AutonomousSimulationResult, Community, SyntheticUser } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isAutonomousModeActive && autonomyEndTime) {
      const interval = setInterval(() => {
        if (Date.now() >= autonomyEndTime) {
          setIsAutonomousModeActive(false);
          setAutonomyEndTime(null);
          setView('workspace'); // Return user to their workspace
        }
      }, 1000); // Check every second
      return () => clearInterval(interval);
    }
  }, [isAutonomousModeActive, autonomyEndTime]);
  ...
  const handleAddGlobalMessage = async (text: string) => {
    if (!user || !activeCommunity) return;
    ...
    // Select a random founding member to respond
    const aiMember = activeCommunity.foundingMembers[Math.floor(Math.random() * activeCommunity.foundingMembers.length)];
    if (!aiMember) return;

    // Simulate typing delay
    setTimeout(async () => {
        try {
            const responseText = await getSyntheticUserResponse(
                text,
                aiMember,
                activeCommunity,
                activeCommunity.globalMessages,
                settings
            );

            const aiResponse: ChatMessage = {
                id: `g-msg-ai-${Date.now()}`,
                user: { // Create a User object from the SyntheticUser for the chat interface
                    name: aiMember.name,
    ...
    switch (view) {
      ...
      case 'research-hub':
        return (
          <ResearchHubPage 
            user={user} 
            messages={activeCommunity.globalMessages} 
            onAddMessage={handleAddGlobalMessage} 
            onlineUsers={activeCommunity.foundingMembers}
            communityName={activeCommunity.name}
            themeDescription={activeCommunity.themeDescription}
          />
        );
      case 'metaswarm':
